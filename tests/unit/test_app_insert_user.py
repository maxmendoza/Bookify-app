...
import json
from user.insert_user.app import lambda_handler, generate_temporary_password, insert_into_user  # Asegúrate de que la ruta sea correcta
from botocore.exceptions import ClientError

class TestInsertUser(unittest.TestCase):

    @patch('user.insert_user.app.get_secret')
    @patch('user.insert_user.app.insert_into_user')
    @patch('user.insert_user.app.boto3.client')
    @patch('user.insert_user.app.generate_temporary_password')
    def test_lambda_handler_success(self, mock_generate_password, mock_boto_client, mock_insert_user, mock_get_secret):
        ...
        self.assertIn('User registered successfully', json.loads(response['body'])['message'])
        ...
        # Verificar que se agregó el usuario al grupo 'Admins'
    ...
    @patch('user.insert_user.app.get_secret')
    @patch('user.insert_user.app.boto3.client')
    def test_lambda_handler_cognito_error(self, mock_boto_client, mock_get_secret):
        mock_get_secret.return_value = {'user_pool_id': 'test_user_pool_id'}
        mock_cognito_client = MagicMock()
        mock_boto_client.return_value = mock_cognito_client
